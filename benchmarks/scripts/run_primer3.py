import json,pathlib,time,platform
import primer3
ROOT=pathlib.Path(__file__).resolve().parents[1]
records=json.loads((ROOT/'data/records.json').read_text(encoding='utf-8-sig'))
...
(ROOT/'results/primer3.json').write_text(json.dumps({'primer3_py':primer3.__version__,'python':platform.python_version(),'config':config,'results':results},indent=2)+'\n')
